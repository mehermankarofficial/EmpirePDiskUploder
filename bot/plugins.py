...
async def search_user_in_community(client, message):
    try:
        await client.get_chat_member('@TheEmpireGroup', message.chat.id)
        await client.get_chat_member('@empirebots', message.chat.id)
    except UserNotParticipant:
        await message.reply_text(not_joined_community, parse_mode = 'html',reply_markup=InlineKeyboardMarkup([
        [InlineKeyboardButton('Join our Channel 1.',url = 'https://t.me/TheEmpireGroup')],
        [InlineKeyboardButton('Join our Channel 2.',url = 'https://t.me/Empirebots')]
        ]))
        return
